...
def tree_norm(tree):
  return jnp.sqrt(tree_sum(jax.tree_util.tree_map(lambda x: jnp.sum(x**2), tree)))
...
def train_step(
  ...
  def loss_fn(variables):
    ...
    weight_l2 = (
        tree_sum(jax.tree_util.tree_map(lambda z: jnp.sum(z**2), variables)) / tree_sum(
            jax.tree_util.tree_map(lambda z: jnp.prod(jnp.array(z.shape)), variables)))
